...
```
D'après le graphique, on constate qu'il y a une légère corrélation entre le tempo et la danceabilité. En effet, plus le tempo est élevé, plus la danseabilité augmente, jusqu'à un certain point. Au-delà de 120 BPM, la danseabilité semble se stabiliser voir même diminuer. Cela peut s'expliquer par le fait que les morceaux très rapides peuvent devenir trop complexes pour être dansés facilement. Ou encore que l'ensemble des données est très concentré vers les 100 BPM, ce qui fausse la perception de la tendance.
...
```
En effet on remarque que la forme est similaire à une gaussienne, avec un maximum vers 100 BPM. Cela peut expliquer la tendance observée précédemment. Il n'y aurait donc pas ou très peu de corrélation entre le tempo et la danseabilité.
...
Dans cette question, nous allons examiner les differentes caracteristiques des musiques pour chaque tonalité. Le but est de voir s'il existe une variation des differentes caracteristiques en fonction de la tonalité globale de la chanson.

**Hypotheses**

Avant de faire des visualisations, nous pouvons supposer que la tonalité a un effet sur certaines caracteristiques, pas toutes. Par exemple, les tonalités majeurs sont plus chaleureuses et festives, tandis que les tonalités minurees donnent un effet plutot nostaligique. Ainsi, nous pouvons supposer que les chansons dans les tonalités majeures vont avoir les features, tel que "danceability" et "energy" plus elevé que les chansons ecrites dans les tonalités mineurs.


**Traitement des donnéees**

Le dataset, tel qu'il est actuellement, n'est pas adapté a faire une seule visualisation representant tous les caracteristiqus en fonction des tonalités. Voici les deux traitement essentielles à faire, avant de passer à l'etape de visualisation : 


- Calcul des moyennes pour chaque feature utilisé : afin de pouvoir afficher les caractristiques des chansons en fonction des tonalité en tant qu'un bar chart, il faut les grouper par tonalité et calculer la moyenne.
- Pivoter le dataset : Afin de colorier les barres par des couleurs differentes en fonction des caractristiques, il faut pivoter le dataset en utilisant la fonction pivot_longer.
```{r}
...
Dans le plan d'analyse initial, nous avons proposé de creer un barplot par tonalité. Cependant, cette methode ne sera pas pertinent pour trouver une correlation. Ainsi, un group bar chart est plus adapté. La plupart des caractristiques sont des scores allant entre 0 et 1, sauf le Tempo (entre 0 et 125BPM) et le Loudness (entre -9 et 0 dB). Ainsi, pour garder une bonne lisibilité, nous avons deecidé de fairee 3 graphes : 1 grouped bar chart avec les scores, 1 bar chart avec le Tempo, et 1 bar chart avec le Loudness.
...
**Observatons diverses**

D'apres les 3 visualisations ci-dessus, nous pouvons constater que la tonalité et les diffrentes n'a pas d'effet sur les caracteristiques musicales.

La question a etait repondu, mais cette reponse n'est pas le meme que dans les hypotheses posés ci-dessus. Ce qui peut etre envisagé, c'est de verifier comment les scores ont etaient calculés par les algorithmes de Spotify et de Youtube.
