...
* les critères de performance sur Youtube ne sont pas corrélés entre eux
* certaines features semblent corrélées entre elles
...
Calcul des moyennes de chaque caracteristique 
```{r}
...
avg_Speechiness <- dataset %>%
  group_by(Key) %>%
  summarise(mean_speechiness = mean(Speechiness, na.rm = TRUE))
...
avg_Instrumentalness <- dataset %>%
  group_by(Key) %>%
  summarise(mean_instrumentalness = mean(Instrumentalness, na.rm = TRUE))

avg_Liveness <- dataset %>%
  group_by(Key) %>%
  summarise(mean_liveness = mean(Liveness, na.rm = TRUE))
...
```{r}
graphe_11_a <- data.frame(avg_Tempo[1], avg_Valence[2], avg_Liveness[2], avg_Instrumentalness[2], avg_Acousticness[2], avg_Speechiness[2], avg_Energy[2], avg_danceability[2])
graphe_11_a <- graphe_11_a |>
  pivot_longer(cols = mean_liveness:mean_danceability,
               names_to = "Type",
               values_to = "Value")
...
ggplot(graphe_11_b, aes(x = Key, y = mean_tempo)) +
  geom_bar(stat = "identity", position = position_dodge(),fill="darkolivegreen4") + 
...
ggplot(graphe_11_c, aes(x = Key, y = mean_loudness)) +
  geom_bar(stat = "identity", position = position_dodge(),fill="deeppink1") + 
