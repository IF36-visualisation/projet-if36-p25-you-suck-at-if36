...
```{r clustering feature selection, message=FALSE, warning=FALSE, echo=FALSE}
library(plotly)
library(dplyr)

# Sélection des features numériques pertinentes
cluster_analysis_data <- dataset %>%
...
cluster_analysis_data_nostream <- cluster_analysis_data %>%
  select(-Stream) %>%
  distinct()
# On garde l’index pour retrouver les infos plus tard
cluster_analysis_data <- cluster_analysis_data[as.numeric(rownames(cluster_analysis_data_nostream)), ]

set.seed(42)
# if (nrow(cluster_analysis_data_nostream) > 2000) { # si plus de 2000 lignes, on échantillonne
#   idx <- sample(nrow(cluster_analysis_data_nostream), 2000) # échantillonnage aléatoire
#   cluster_analysis_data_nostream <- cluster_analysis_data_nostream[idx, ] # échantillonnage
#   cluster_analysis_data <- cluster_analysis_data[idx, ] # on garde les infos des streams
# }
```
...
umap_result <- umap(as.matrix(cluster_analysis_data_nostream), config = umap_config)
umap_df <- as.data.frame(umap_result$layout)
colnames(umap_df) <- c("UMAP1", "UMAP2")

# Ajout des infos
umap_df$Title <- dataset$Track[as.numeric(rownames(cluster_analysis_data))]
umap_df$Artist <- dataset$Artist[as.numeric(rownames(cluster_analysis_data))]
umap_df$Stream <- cluster_analysis_data$Stream
...
```{r cluster analysis with dbscan, echo=FALSE}
# LIBRAIRIES
library(dplyr)
library(dbscan)
library(Rtsne)
...
features <- cluster_analysis_data_nostream
...
# Trouver un epsilon adapté peut nécessiter un test (à ajuster si besoin)
db_result <- dbscan(features_scaled, eps = 0.6, minPts = 10)

# Ajouter les clusters au dataset original
features$Cluster <- factor(db_result$cluster) # 0 = bruit (outliers)
dataset_clustered <- dataset[as.numeric(rownames(features)), ]
dataset_clustered$Cluster <- features$Cluster

# 3. VISUALISATION AVEC t-SNE -------------------

set.seed(123)
features_scaled_nodup <- features_scaled[!duplicated(features_scaled), ] # retire les doublons
features_nodup <- features[!duplicated(features_scaled), ] # pour garder la correspondance

tsne_result <- Rtsne(features_scaled_nodup, dims = 2)
tsne_df <- as.data.frame(tsne_result$Y)
colnames(tsne_df) <- c("Dim1", "Dim2")

# Utilise les index des lignes non dupliquées pour récupérer les bons titres/artistes
row_idx <- as.numeric(rownames(features_nodup))
dataset_clustered_nodup <- dataset_clustered[row_idx, ]

tsne_df$Cluster <- features_nodup$Cluster
tsne_df$Title <- dataset_clustered_nodup$Track
tsne_df$Artist <- dataset_clustered_nodup$Artist
...
```{r cluster_plot, echo=FALSE, message=FALSE, warning=FALSE}
library(plotly)

# Crée une liste de traces, une par cluster
traces <- lapply(unique(tsne_df$Cluster), function(clust) {
  cluster_data <- tsne_df %>% filter(Cluster == clust)
  list(
    x = cluster_data$Dim1,
    y = cluster_data$Dim2,
    type = "scatter",
...
# Affiche toutes les traces avec un menu pour filtrer
fig <- plot_ly()
for (t in traces) {
...
buttons <- lapply(seq_along(traces), function(i) {
  l <- rep(FALSE, length(traces))
  l[i] <- TRUE
  list(
    method = "restyle",
    args = list("visible", l),
    label = paste("Cluster", unique(tsne_df$Cluster)[i])
  )
})

# Bouton pour tout afficher
buttons <- append(
  list(list(method = "restyle", args = list("visible", rep(TRUE, length(traces))), label = "Tous")),
  buttons
)

# Ajout du layout avec menu
fig <- fig %>% layout(
  title = "Clustering des chansons (filtrable par cluster)",
...
# 4. ANALYSE AUTOMATIQUE DES GROUPES -----------

cat("\nRésumé automatique par cluster :\n\n")

summary_table <- dataset_clustered %>%
  filter(Cluster != 0) %>% # exclure les outliers
  group_by(Cluster) %>%
  summarise(
    n = n(),
