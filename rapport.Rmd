...
Dans le cadre de ce projet, nous avons utilisé plusieurs bibliothèques, afin d'analyser, traiter et visualiser les données. Les bibliothèques utilisées sont : *ggplot2, scales, readr, dplyr, magrittr, tidyr, boot, forcats, hexbin* et *fmsb*.
...
### Thomas
Ce projet m’a donné l'occasion analyser les données de manière critique, notamment en identifiant les limites des features utilisées et les biais présents dans le dataset. J’aurais aimé aller plus loin en explorant des approches complémentaires, comme l’intégration de données qualitatives ou l’analyse temporelle des performances des chansons. Ce projet m’aura montré l’importance de la qualité des données pour obtenir des résultats pertinents et exploitables.
